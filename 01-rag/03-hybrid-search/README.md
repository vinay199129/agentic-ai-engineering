...
See [`eval-snapshot.json`](./eval-snapshot.json) for per-retriever recall@{1,3,5}. Compare against [`../00-naive-rag/eval-snapshot.json`](../00-naive-rag/eval-snapshot.json) for the dense-only baseline.
